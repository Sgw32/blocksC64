...
rayCast		jsr getUnitXDist			; Get the distance with X=1 unit long
			...
			ldx #0	
			lda player_PosX+1			; Take the remainder of the player X position

			bit raySignX				; Verify if ray direction is negative, 
			bmi rayCast_isNX					; BIT opcode will match N and V flags from the target
...
rayCast_doDDE		lda player_PosX+2			; Prepare mapX and mapY positions
			sta rayMapX
			lda player_PosY+2
			sta rayMapY
...
;rayCast_brk1
rayCast_loop	  	lda rayDistX				; Find the shortest distance
			cmp rayDistY
			lda rayDistX+1
			sbc rayDistY+1
			bcc rayCast_XLTY
			...
			;cmp #$20					; If not empty the go to rayCast_cellFound
			;bne rayCast_cellFoundY
			cmp #128					; Anything below code 128 is empty space
			bcs rayCast_cellFoundY			
			...
			dec B
			bne rayCast_loop
			jmp rayCast_notFound    

			;  If rayDistX >= rayDistY then step in y direction

rayCast_XLTY  		lda raySignX				; mapX = mapX + raySignX ... where raySignX = can be -1 or +1
			...
			;cmp #$20
			;bne rayCast_cellFoundX
			cmp #128					; Anything below code 128 is empty space
			bcs rayCast_cellFoundX				
...
rayCast_cellFoundX	lda #1
			...
			lda screen_ViewMode			; If 2D then calculate rayDeltaY
			beq rayCast_calcDeltaY
			rts
...
rayCast_calcDeltaY	
			...
			lda player_absHeadSin
			sta DL
			lda player_absHeadSin+1
			sta DH
...
rayCast_cellFoundY	lda #2
			...
			lda screen_ViewMode			; If 2D then calculate rayDeltaX
			beq rayCast_calcDeltaX
			rts
...
rayCast_calcDeltaX 
			...
			lda player_absHeadCos
			sta DL
			lda player_absHeadCos+1
			sta DH
...
getUnitXDist
			...
			bne getUnitXDist_notSpecial				; If lo-byte is 0 then it is a special case.

			; Special case for X axis (90° = $0100 Ticks, 270° = $0300 Ticks)
	
			bcc getUnitXDist_read					; Only odd quadrants. Skip if even.
	
			lda #255
			sta rayUnitXDist			; If the quadrant is odd (hi-byte) and
			sta rayUnitXDist+1			; the lo-byte is zero, then this it is
			jmp getUnitXDist_setSign				; a special case (90°or 270°).  

getUnitXDist_notSpecial	bcc getUnitXDist_read					; If quadrant is even then read from the front.
...
getUnitXDist_read		lda slopeLen_lo,y
			sta rayUnitXDist			; Distance traveled with X=1 unit length
			lda slopeLen_hi,y
			sta rayUnitXDist+1			; Distance traveled with X=1 unit length

getUnitXDist_setSign	ldy rayHeading+1			; Get quadrant
			lda getUnitXDist_tblSignX,y
...
getUnitXDist_tblSignX	!byte  1,-1,-1, 1 			 
...
getUnitYDist
			...
			ldy rayHeading				; Carry is still preserved    
			bne getUnitYDist_notSpecial				; If lo-byte is 0 then it is a special case.

			; Special case for X axis (0° = $0000 Tick, 180° = $0200 Tick)

			bcs getUnitYDist_read					; Only even quadrant. Skip if odd.
	
			lda #255
			sta rayUnitYDist			; If the quadrant is even (hi-byte) and
			sta rayUnitYDist+1			; the lo-byte is zero, then this it is
			jmp getUnitYDist_setSign				; a special case (0°or 180°).  

getUnitYDist_notSpecial	bcs getUnitYDist_read					; If quadrant is odd then read from the front.
...
getUnitYDist_read		lda slopeLen_lo,y
			sta rayUnitYDist
			lda slopeLen_hi,y
			sta rayUnitYDist+1  

getUnitYDist_setSign	ldy rayHeading+1			; Get the quadrant
			lda getUnitYDist_tblSignY,y
			sta raySignY
...
getUnitYDist_tblSignY	!byte -1,-1, 1, 1 			; INVERTED!!!   
...
slopeLen_hi		!byte 001,001,001,001,001,001,001,001,001,001,001,001,001,001,001,001
...
slopeLen_lo		!byte 000,000,000,000,000,000,000,000,000,000,000,001,001,001,001,001
