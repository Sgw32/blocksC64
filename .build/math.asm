...
sinus 		
			...
			bcc sinus_readFwd				; then read the table from the front.

			; Special case (angle 90 and 270 - just for negative quadrants)

			tya							; Y back to A
			bne sinus_notSpec				; If A = 0 then it is special
			inc DH						; Angle 90 = quadrant 1 (negative)
			inc EH
			bne sinus_setSign				; Angle 270 = quadrant 3 (negative)

			; If not special then read table from the back

sinus_notSpec	eor #255					; Reversing table reading direction
			tay							; Two's complement
			iny

  			; Read table from the front
  
sinus_readFwd	lda tblSine,y
			sta DL
		    sta EL 
   
			; Get sign from quadrants (X contains hi-byte, i.e. quadrant)
  
sinus_setSign	lda sinus_sinSign,x				; Return sign in Y register
			bpl sinus_exit					; Don't negate if positive.
...
sinus_exit	  	rts							; Second return (for odd cases)  

			; Local variables and tables

sinus_sinSign	!byte -1,-1, 1, 1			; REVERSED!!!!
...
cosinus_notSpec  	eor #255					; Reversing table reading direction
			tay
			iny
  
cosinus_readFwd 	lda tblSine,y
			sta DL 
			sta EL
  
			; Get sign from quadrants (X contains hi-byte, i.e. quadrant)
  
cosinus_setSign	lda cosinus_cosSign,x		
			bpl cosinus_exit					; Don't negate if positive.
...
multiply_loop 		asl DL
			rol DH
			rol EL

multiply_enter		lsr CH
			ror CL
			bcs multiply_doAdd 					; If the number was odd then do addition

			lda CH
			ora CL
			bne multiply_loop					; If the number <> 0 then do loop
